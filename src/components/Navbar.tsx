...
import { Trophy, Calendar, Users, Award, Home, Sun, Moon } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 bg-[var(--page)] border-b-2 border-[var(--line)]">
        ...
        <div className="flex items-center justify-between h-16 sm:h-20 gap-2">
          ...
          >
            <button
              ...
              className="w-10 h-10 sm:w-12 sm:h-12 bg-[var(--surface-soft)] border-2 border-[var(--line)] flex items-center justify-center shadow-sm group-hover:translate-x-0.5 group-hover:translate-y-0.5 group-hover:shadow-sm transition-all overflow-hidden focus:outline-none focus:ring-2 focus:ring-[var(--accent)] cursor-zoom-in"
              title="Ampliar logo"
            >
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="font-display text-xl sm:text-2xl font-black text-[var(--ink)] tracking-wider leading-none">
                  RANKING PADEL
                </span>
              </div>
              <p className="text-[11px] font-mono-code text-[var(--muted)] hidden sm:block tracking-tight mt-0.5">
          ...
          <nav className="hidden lg:flex items-center gap-1.5 bg-[var(--surface)] p-1.5 border-2 border-[var(--line)]">
            {tabs.map((tab) => {
              const Icon = tab.icon;
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-3.5 py-2 text-xs font-black uppercase font-grotesk tracking-wider transition-all duration-100 ${
                    isActive
                      ? 'bg-[var(--accent)] text-black border border-[var(--line)] shadow-sm'
                      : 'text-[var(--ink)] hover:text-[var(--ink)] hover:bg-[var(--surface-raised)]'
                  }`}
                >
                  <Icon className={`w-3.5 h-3.5 ${isActive ? 'text-black' : 'text-[var(--muted)]'}`} />
                  {tab.label}
                  {'tag' in tab && (
                    <span className={`text-[9px] px-1 py-0.2 font-black ${
                      isActive ? 'bg-[var(--surface-soft)] text-[var(--accent-ink)]' : 'bg-[var(--accent)] text-black'
                    }`}>
                      {tab.tag}
                    </span>
                  )}
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-[var(--page)] border-t-2 border-[var(--line)] pt-1 pb-[calc(0.35rem+env(safe-area-inset-bottom))] px-2">
            ...
            return (
              <button
                ...
                className={`min-h-14 flex flex-col items-center justify-center py-1.5 px-0.5 font-grotesk uppercase transition-all ${
                  isActive
                    ? 'text-black bg-[var(--accent)] font-black border border-[var(--line)]'
                    : 'text-[var(--muted)] hover:text-[var(--ink)]'
                }`}
              >
                <Icon className={`w-4 h-4 mb-0.5 ${isActive ? 'text-black' : 'text-[var(--muted)]'}`} />
