...
export const PlayerEvolution: React.FC<PlayerEvolutionProps> = ({ history, totalPlayers }) => {
  ...
  if (!history.length) {
    return (
      <div className="bg-black border-2 border-[var(--line)] p-4 text-center text-slate-400">
  ...
  return (
    <section className="bg-[var(--surface)] border-2 border-black shadow-[4px_4px_0px_0px_var(--accent)] p-4 sm:p-5">
      <div className="flex items-center justify-between gap-3 mb-4">
        <div>
          <span className="bg-[var(--accent)] text-black text-[10px] font-black px-2 py-0.5 uppercase font-grotesk border border-black">
            Evolución histórica
          </span>
      ...
      <div className="overflow-x-auto border-2 border-[var(--line)] bg-black p-3">
        <svg width={width} height={height} role="img" aria-label="Gráfica de evolución de posición">
          ...
          <path d={path} fill="none" stroke="var(--accent)" strokeWidth="4" strokeLinecap="square" strokeLinejoin="round" />
          {points.map((point) => (
      ...
      <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
        {history.map((point) => (
          <div key={point.roundNumber} className="bg-black border border-[var(--line)] px-3 py-2 flex items-center justify-between">
